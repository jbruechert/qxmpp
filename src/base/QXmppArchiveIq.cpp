...
void QXmppArchiveChat::parse(const QDomElement &element)
{
    ...
    while (!child.isNull())
    {
        if ((child.tagName() == QLatin1String("from")) || (child.tagName() == QLatin1String("to")))
        {
            ...
            message.setReceived(child.tagName() == QLatin1String("from"));
            m_messages << message;
        }
...
void QXmppArchiveChat::toXml(QXmlStreamWriter *writer, const QXmppResultSetReply &rsm) const
{
    ...
    writer->writeAttribute(QStringLiteral("xmlns"), ns_archive);
...
void QXmppArchiveListIq::parseElementFromChild(const QDomElement &element)
{
    ...
    while (!child.isNull())
    {
        if (child.tagName() == QLatin1String("chat"))
        {
