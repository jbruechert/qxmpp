...
bool QXmppStream::sendPacket(const QXmppStanza &stanza)
{
    ...
    return sendPacketAsync(stanza).resultAt(0) != QXmpp::SocketError;
}
...
/// \since QXmpp 1.4
///
QFuture<QXmpp::PacketState> QXmppStream::sendPacketAsync(const QXmppStanza &stanza)
{
...
{
    if (iq.id().isEmpty()) {
        warning(QStringLiteral("Could not send IQ: ID is empty."));
        return {};
    }
    if (d->runningIqs.contains(iq.id())) {
        warning(QStringLiteral("Could not send IQ: "
                               "An IQ request with this ID (\"%1\") is still ongoing.")
                    .arg(iq.id()));
        return {};
    }

    auto *interface = new IqState(sendPacketAsync(iq), d->streamManager.enabled());
...
void QXmppStream::sendPacket(QXmppPacket &packet)
{
    if (sendData(packet.data())) {
        ...
        packet.reportResult(QXmpp::SocketError);
    }
}
